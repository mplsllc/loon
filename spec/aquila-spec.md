# Aquila Language Specification

Version 0.1 — Draft

## 1. Overview

Aquila is a programming language designed for AI coding agents as the primary author. Its primary design goal: make it structurally impossible to write code that leaks secrets, ignores errors, or breaks production — whether the author is a human, an LLM, or both.
...
Aquila source files are UTF-8 encoded. The lexer operates on bytes. Identifiers and keywords use ASCII letters, digits, and underscores. String literals may contain any UTF-8 sequence.
...
Every Aquila source file is a module. The module declaration must be first.
...
Every source file is a module. The module name must match the filename (without `.aquila` extension).
...
In v0.1, `imports [math]` looks for `math.aquila` in the same directory as the importing file. No search paths, no package system yet.
...
```json
{
    ...
    "code": "E0012",
    "location": {"file": "main.aquila", "line": 15, "column": 5},
