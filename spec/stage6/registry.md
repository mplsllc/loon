# Akvila Package Registry — Stage 6.2 Specification

## Overview

The Akvila package registry has two tiers matching the MPLS License values:
...
```bash
akvila init           # create new project with akvila.pkg
akvila add crypto     # add dependency
akvila build          # compile with dependencies
akvila test           # run tests
akvila publish        # publish to registry
akvila search json    # search packages
```
...
Simple version pinning for v1. Each project has a `akvila.lock` file with exact versions. Reproducible builds by default.

```
# akvila.lock — auto-generated, committed to version control
crypto = "0.2.1"
http = "0.1.3"
json = "0.1.0"
```
...
3. Published packages have a valid akvila.pkg manifest
...
1. Simple HTTP API (can be written in Akvila with FFI for HTTP)
2. Package storage on disk (git-based, like crates.io index)
3. Authentication via GitHub OAuth

### Phase 2: CLI tool

1. `akvila-pkg` already exists — extend with registry integration
2. `akvila add` fetches from registry, stores in `.akvila_packages/`
3. `akvila publish` uploads to registry with authentication
...
3. Implement `json` in pure Akvila (no FFI needed)
