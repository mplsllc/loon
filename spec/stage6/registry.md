# Aquila Package Registry — Stage 6.2 Specification

## Overview

The Aquila package registry has two tiers matching the MPLS License values:
...
```bash
aquila init           # create new project with aquila.pkg
aquila add crypto     # add dependency
aquila build          # compile with dependencies
aquila test           # run tests
aquila publish        # publish to registry
aquila search json    # search packages
```
...
Simple version pinning for v1. Each project has a `aquila.lock` file with exact versions. Reproducible builds by default.

```
# aquila.lock — auto-generated, committed to version control
crypto = "0.2.1"
http = "0.1.3"
json = "0.1.0"
```
...
3. Published packages have a valid aquila.pkg manifest
...
1. Simple HTTP API (can be written in Aquila with FFI for HTTP)
2. Package storage on disk (git-based, like crates.io index)
3. Authentication via GitHub OAuth

### Phase 2: CLI tool

1. `aquila-pkg` already exists — extend with registry integration
2. `aquila add` fetches from registry, stores in `.aquila_packages/`
3. `aquila publish` uploads to registry with authentication
...
3. Implement `json` in pure Aquila (no FFI needed)
