# Akvila-0 Language Specification

Version 0.1 — Bootstrap Subset

## 1. Overview

Akvila-0 is the minimal subset of Akvila needed to write a compiler. It has 14 constructs, 4 types, and 8 built-in functions. Every construct earns its place by being necessary for Stage 2 (the real Akvila compiler written in Akvila-0).

Akvila-0 is compiled by Stage 1 (parser + codegen in x86-64 assembly). Once Stage 2 exists, Akvila-0 and Stage 1 are never used again.

## 2. Differences from Full Akvila

Akvila-0 is a strict subset. Nothing in Akvila-0 contradicts the full Akvila spec — it only omits features.

| Feature | Full Akvila | Akvila-0 | Reason |
|---------|-----------|--------|--------|
...
Akvila-0 uses the same lexer as full Akvila (Stage 0). All tokens defined in the full Akvila spec are valid input to the Stage 1 compiler, but only the subset below is recognized by the parser.
...
9 of the 13 full Akvila keywords. `type`, `imports`, `exports`, and `sequential` are not recognized by the Akvila-0 parser (they produce a parse error if encountered at the top level).
...
Not used in Akvila-0: `|>` (pipe), `.` (field access).
...
A Akvila-0 program is a module declaration followed by one or more function declarations. No top-level `let` bindings. No `type` declarations.
...
`array_set_stmt` is the one form of mutation in Akvila-0 — arrays are mutable containers (like buffers). The array name must be an `IDENT` (not an arbitrary expression). This is necessary for a compiler that builds token and node arrays.
...
Where `IDENT` as a pattern is the wildcard `_` (the only identifier valid as a pattern in Akvila-0). No ADT variant patterns. No string patterns.
...
`do` is a unary prefix operator. `do f(x)` and `f(x)` produce identical compiled code. The `do` keyword exists in the syntax so that Akvila-0 code remains valid full Akvila code — removing it would create programs that don't compile under the real Akvila compiler's effect checker.
...
### 5.2 Type Checking in Akvila-0

The Akvila-0 "type checker" is minimal:
...
3. **No type compatibility checking.** `let x: Int = "hello";` is not caught by Akvila-0. Full type checking is a Stage 2 concern.
...
Arrays are `(pointer, length)` pairs. The pointer points into the bump heap. Each element is 8 bytes (one `Int`, one `Bool`, or one string pointer — all values are 8 bytes or 16 bytes, but array elements store only the first 8 bytes, so arrays of String or Array are not supported in Akvila-0).
...
Arrays store 8-byte values only. Storing a String or Array value (which is 16 bytes) in an array element is not caught by the Akvila-0 type checker. Behavior is undefined. In practice, only `Array<Int>` is used in Akvila-0 programs.
...
Akvila-0 uses lexical scoping with no shadowing.
...
Shadowing (declaring a variable with the same name as an existing one in an enclosing scope) is a compile error in Akvila-0. This restriction simplifies stack slot assignment in codegen.
...
```
source.akvila → stage0/lexer → token stream → stage1/compiler → output.asm → nasm → ld → binary
```
...
Compiled Akvila-0 functions use the AMD64 System V ABI:
...
The `+` operator is overloaded between Int and String in Akvila-0 (this is the only operator overloading, inherited from full Akvila). The AST node for `BINOP ADD` carries a `type_info` field:

- `type_info == 0` (Int): emit `add rax, rcx`
- `type_info == 2` (String): emit call to `str_concat` runtime function

The parser sets `type_info` on `BINOP ADD` nodes: if either operand is a string literal (`NODE_STR_LIT`) or a variable declared with type `String`, `type_info = 2` (String). Otherwise `type_info = 0` (Int). This is the only type propagation the Akvila-0 parser performs.
...
Compiled Akvila-0 programs may produce runtime errors:
...
Expected output: `Hello, Akvila!` followed by newline. Exit code 0.
...
Expected output: `Hello, Akvila!`. Exit code 0.
...
Note: `for` loops are side-effect-only in Akvila-0. Scalar accumulation requires `fold` (Stage 2). Array accumulation uses `arr[i] = expr` (section 4.5).
...
This is `examples/effects.akvila` — the Stage 1 completion test.

## 11. What Is Not in Akvila-0
...
| Effect verification | `do` is syntactic only in Akvila-0 | Full effect set tracking and verification |
...
| `return` keyword | Not in Akvila spec — last expression is return value | N/A — stays out |
| `type` declarations | No custom types in Akvila-0 | Full ADT declarations |
| Closures | No first-class functions | Anonymous functions with captures |
| Variable shadowing | Simplifies stack slot assignment | Allowed in full Akvila |
| `sequential` blocks | Everything is sequential in Akvila-0 | Parallel-by-default with `sequential` as exception |
