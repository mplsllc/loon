# Akvila Language Specification

Version 0.1 — Draft

## 1. Overview

Akvila is a programming language designed for AI coding agents as the primary author. Its primary design goal: make it structurally impossible to write code that leaks secrets, ignores errors, or breaks production — whether the author is a human, an LLM, or both.
...
Akvila source files are UTF-8 encoded. The lexer operates on bytes. Identifiers and keywords use ASCII letters, digits, and underscores. String literals may contain any UTF-8 sequence.
...
Every Akvila source file is a module. The module declaration must be first.
...
Every source file is a module. The module name must match the filename (without `.akvila` extension).
...
In v0.1, `imports [math]` looks for `math.akvila` in the same directory as the importing file. No search paths, no package system yet.
...
```json
{
    ...
    "code": "E0012",
    "location": {"file": "main.akvila", "line": 15, "column": 5},
