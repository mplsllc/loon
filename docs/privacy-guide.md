...
Aquila makes that constraint part of the type system. `Sensitive<String>` is not `String`. The compiler treats them as different types. Operations that are safe for `String` (logging, serialization, display) are compile errors for `Sensitive<String>`.

## The privacy levels

Every value in Aquila has a privacy level. The default is Raw (unclassified).
...
Certain cryptographic algorithms are known to be broken. Aquila doesn't provide them:
