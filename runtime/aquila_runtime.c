// Aquila LLVM runtime — provides builtins that the LLVM IR backend declares as external
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
char* _aquila_i64_to_str(long long val) {
    char* buf = malloc(24);
    sprintf(buf, "%lld", val);
    return buf;
}
...
char* _aquila_get_arg(long long n) {
...
__attribute__((constructor)) static void _aquila_init_args(void) {
