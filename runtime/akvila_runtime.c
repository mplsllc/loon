// Akvila LLVM runtime — provides builtins that the LLVM IR backend declares as external
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
char* _akvila_i64_to_str(long long val) {
    char* buf = malloc(24);
    sprintf(buf, "%lld", val);
    return buf;
}
...
char* _akvila_get_arg(long long n) {
...
__attribute__((constructor)) static void _akvila_init_args(void) {
