#!/bin/bash
# LLM Agent Gauntlet — measures what Akvila catches when AI writes security-critical code
#
# Two variants per prompt:
#   *_untyped.akvila — naive implementation, no privacy types (simulates Python/Go-like code)
#   *_typed.akvila   — using Sensitive<String> but still making mistakes
#
# Results: COMPILED (vulnerability shipped) or CAUGHT (compiler stopped it)

COMPILER="${AKVILA_COMPILER:-/tmp/s2_new2}"
CAUGHT=0
COMPILED=0
TOTAL=0
...
for test in "$DIR"/prompt*.akvila; do
    TOTAL=$((TOTAL + 1))
    name=$(basename "$test" .akvila)
...
echo "────────────────────────────────────────"
echo ""
echo "  'SHIPPED' = this code compiles in Akvila. If an AI agent wrote it,"
echo "  the vulnerability reaches production unless a human catches it."
echo ""
echo "  'CAUGHT' = the Akvila compiler rejected this code. The AI agent"
...
echo "  compile because Akvila's privacy system is opt-in at the type"
