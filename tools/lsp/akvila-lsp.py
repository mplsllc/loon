#!/usr/bin/env python3
"""Akvila Language Server — minimal LSP implementation.

Provides diagnostics (errors) for .akvila files in any LSP-compatible editor.
Uses the Akvila compiler's --json flag for structured error output.
"""
...
AKVILA_COMPILER = os.environ.get("AKVILA_COMPILER", "akvila")
...
def get_diagnostics(file_path):
    """Run the Akvila compiler and extract diagnostics."""
    diagnostics = []
    try:
        result = subprocess.run(
            [AKVILA_COMPILER, "--json", file_path],
        ...
        diagnostics.append({
            ...
            "severity": 1, "source": "akvila",
            "message": f"Akvila compiler error: {e}"
        })
    return diagnostics
        ...
        if method == "initialize":
            send_message({
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "akvila-lsp", "version": "0.1.0"}
                }
            })
