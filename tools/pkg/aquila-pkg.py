#!/usr/bin/env python3
"""Aquila Package Manager — minimal implementation.

Commands:
  aquila-pkg init          Create a new aquila.pkg manifest
  aquila-pkg build         Compile the project with dependencies
  aquila-pkg add <name>    Add a dependency (from registry or local path)
  aquila-pkg list          List installed dependencies
"""
...
REGISTRY_URL = "https://pkg.aquila-lang.org"  # Future registry
PKG_DIR = ".aquila_packages"


def load_manifest(path="aquila.pkg"):
    """Load the package manifest."""
    if not os.path.exists(path):
        print(f"error: {path} not found. Run 'aquila-pkg init' first.")
        sys.exit(1)
    manifest = {}
    with open(path) as f:
        for line in f:
...
def cmd_init():
    """Create a new aquila.pkg manifest."""
    if os.path.exists("aquila.pkg"):
        print("aquila.pkg already exists.")
        return
    name = os.path.basename(os.getcwd())
    with open("aquila.pkg", "w") as f:
        f.write(f'// Aquila package manifest\n')
        f.write(f'name = "{name}";\n')
        f.write(f'version = "0.1.0";\n')
        f.write(f'license = "MPLS-1.0";\n')
        f.write(f'main = "src/main.aquila";\n')
        f.write(f'dependencies = [];\n')
    print(f"Created aquila.pkg for '{name}'")


def cmd_build():
    """Build the project."""
    manifest = load_manifest()
    main_file = manifest.get("main", "src/main.aquila")
    if not os.path.exists(main_file):
        print(f"error: main file '{main_file}' not found")
        sys.exit(1)

    compiler = os.environ.get("AQUILA_COMPILER", "aquila")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: aquila-pkg <command> [args]")
