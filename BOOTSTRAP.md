# How Akvila Bootstrapped from Bare Metal
...
**Goal:** Read a `.akvila` source file and emit tokens to stdout.
...
**Build pipeline:**
```bash
./stage0/lexer input.akvila | ./stage1/compiler > output.asm
nasm -f elf64 -o output.o output.asm && ld -o output output.o
```

After M2.2, the first compiled Akvila program ran. `strace` showed:

```
execve("/tmp/effects", ...)
write(1, "Sum: 7", 6)
write(1, "\n", 1)
write(1, "Hello, Akvila", 11)
write(1, "\n", 1)
exit(0)
```
...
## Stage 2 — Akvila written in Akvila

**Goal:** Rewrite the compiler in Akvila-0 (the bootstrap subset), compile it with Stage 1, then compile itself.

### M2.0–M2.2: Translation

The first three milestones were direct translation — the same lexer, parser, and codegen logic rewritten in Akvila-0 instead of assembly. Each line of Akvila-0 replaced 3-5 lines of assembly. The compiler shrank from 7,355 lines of assembly to ~1,200 lines of Akvila-0.

Akvila-0 is deliberately minimal: `fn`, `let`, `match` on Int/Bool, `for`, arrays, strings. No ADTs, no generics, no closures. The compiler used flat integer arrays with manual offset arithmetic — the same pattern as Stage 1, just readable.
...
The security gauntlet tested every class of violation Akvila promises to prevent:
...
Stage 2:  1,956 lines of Akvila-0 (self-hosting compiler)
...
Real programs:  3 (akvila_call, calculator, security_gauntlet)
