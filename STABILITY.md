# Akvila Stability Policy

## Current Status

Akvila is pre-1.0 software. It works. It self-hosts. It passes its full test
...
Akvila uses [Semantic Versioning](https://semver.org/) with the following
interpretation:
...
**Syntax.** The concrete syntax of Akvila may change before 1.0. Keyword names,
operator precedence, and expression forms are all subject to revision. If you
write Akvila code today, expect to update it.
...
current pipeline (`lexer input.akvila | compiler > output.asm`) reflects the
...
When Akvila reaches 1.0, the following guarantees will apply:
...
Akvila is released under the MPLS Principled Libre Software License.
